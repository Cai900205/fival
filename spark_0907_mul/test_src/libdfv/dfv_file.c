...
static void* __dfv_slice_worker(void* arg)
{
    ...
    int slice_num = slice_ctx->total_slice_num;
...
dfv_file_t* dfv_file_open(dfv_slot_def_t* slot_def, SPK_DIR dir, dfv_slice_def_t* slice_def, int cpu_base)
{
    ...
    for (j=0; j<repo_num; j++) {
        ...
        file_ctx->dir = dir;
        file_ctx->total_slice_num = (fmeta->slice_num * repo_num);
        // spawn workers
        for (i=0; i<fmeta->slice_num; i++) {
            int slice_index = i + (j * fmeta->slice_num);
            ...
            slice_ctx->total_slice_num = (fmeta->slice_num * repo_num);
...
int dfv_file_close(dfv_file_t* file_ctx)
{
    ...
    int i;
    size_t slice_num = file_ctx->total_slice_num;
    ...
    for(i=0; i<file_ctx->repo_num; i++) {  
        if (file_ctx->dir == SPK_DIR_WRITE) {
            if (file_ctx->fmeta[i]->file_pos > 0) {
                ...
                if (slot_sz < 0) {
                    ...
                    file_ctx->fmeta[i]->slot_sz = slot_sz;
        ...
        // dec open_cnt
        pthread_mutex_lock(&file_ctx->fmeta[i]->open_cnt_lock);
        file_ctx->fmeta[i]->open_cnt--;
        pthread_mutex_unlock(&file_ctx->fmeta[i]->open_cnt_lock);
        if (remove_slot) {
...
{
    int slice_num = file_ctx->total_slice_num;
    int i = 0;
//    assert(file_ctx->dir == SPK_DIR_READ);
    ...
    for(i=0; i<file_ctx->repo_num; i++) {  
        file_ctx->fmeta[i]->file_pos = offset;
