    ...
    while (parse_argc+2 < argc) {
        ...
        do {
            ...
            if (key) {
                ...
                } else if (!strcasecmp(key, "--file-sizeG")) {
                    size_t file_sz_in_kb = strtoul(value, NULL, 0);
                    file_size = file_sz_in_kb * 1024 *1024 *1024;
                } else {
...
//ctx add
    ...
    vault = dfv_vault_open(repo_mnt_num, repo_mnt_tbl, repo_dev_tbl, 0);
    if (!vault) {
        printf("Failed to create vault, mnt_num:%d\n", repo_mnt_num);
        exit(1);
    }
    ...
    if (!strcasecmp(cmd, "flist")) {
        ...
        while(xferred < file_size) {
            size_t xfer = MIN(chunk_size, file_size-xferred);
